...
public class EntityPlacerBlock extends Block
{
	public static final PropertyEnum<EnumFacing> PLACER_FACING = PropertyEnum.create("placer_facing", EnumFacing.class);
	...
	public EntityPlacerBlock()
	{
		super(Material.GROUND);
		this.blockName = "entity_vacuum";
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(PLACER_FACING, EnumFacing.DOWN));
	}
	...
	public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player, boolean willHarvest)
	{
		if(!world.isRemote)
		{
			IPipeNetwork network = PipeNetworkManager.getNetworkManagerForType(NetworkType.ENTITY).getNetwork(pos, world.provider.getDimension());
			if(network != null)
				for(EnumFacing side : EnumFacing.VALUES)
					network.removeInterfacedBlock(world, pos, side.getOpposite());

			PipeNetworkManager.getNetworkManagerForType(NetworkType.ENTITY).removePipeFromNetwork(world, pos);
		}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	{
		if(!world.isRemote)
		{
			IPipeNetwork network = PipeNetworkManager.getNetworkManagerForType(NetworkType.ENTITY).addPipeToNetwork(world, pos);
			EnumFacing side = state.getValue(PLACER_FACING).getOpposite();
	...
	{
		InterfaceFilter filter = new InterfaceFilter(side.getOpposite(), NetworkType.ENTITY);
	...
	{
		return new BlockStateContainer(this, new IProperty[] { PLACER_FACING, NORTH, EAST, SOUTH, WEST, UP, DOWN });
	}
}
